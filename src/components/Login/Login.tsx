...
import { collection, getFirestore, query, where, and, getDocs } from "firebase/firestore";
...
/**
 * 
 * @param setLoggedIn Function that updates the isLoggedIn state property in App.tsx when the user successfully logs in 
 * @returns JSX Component - Login form
 */
export default function Login({setLoggedIn}:{setLoggedIn: (isLoggedIn : boolean) => void}){

  const [errMsg, throwErr] = useState("‎");
  const [login, setLogin] = useState("");
  const [pass, setPass] = useState("");

  /**
   * Sets state login and password to ones provided by the user. Hashes the password using MD5 
  ...
  const inputHandler = (e: React.FormEvent<HTMLInputElement>) => {
    switch ((e.target as HTMLInputElement).name) {
      ...
      default:
        setPass(MD5(MD5((e.target as HTMLInputElement).value)+"Qwerty1@3"))
        break;
    }
  }

  /**
   * Connects to Firestore DB, checks if login and password match.
   */

  const loginToApp = ()=>{
    const db = getFirestore(app);
    const adminAccs = collection(db, "privilegedAccounts")
    const q = query(adminAccs, and(where('login', '==', login), where('password', '==', pass)));

    getDocs(q)
    .then((result)=>{
      if(result.size != 1){
        throwErr("Invalid login or password")
        return;
      }
      const date: Date = new Date();
      date.setTime(date.getTime() + 10*60*1000)
      document.cookie = `loggedIn=;expires=${date.toUTCString()};path=/`;
      setLoggedIn(true);
    })
    .catch((err)=>{
      throwErr("Can't connect to the database. Check your internet connection or try again later")
      console.log(err)
    });
  ...
  return(
    <>
      <img src={Logo} alt="logo-robot" className="login-logo"/>
      <div className="form">
        <h2>Key-Up Center<br/>for Companies</h2>
        <Input label="Login" name="login" inputHandler={inputHandler}/>
        <Input label="Password" name="password" inputHandler={inputHandler} type="password"/>
        <Button text="Log in" clickHandler={loginToApp}/>
        <p>{errMsg}</p>
      </div>
